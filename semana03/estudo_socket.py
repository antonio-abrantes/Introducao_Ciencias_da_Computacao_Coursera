...
while True:
    ...
    try:
        ...
        if "{0}:{1}".format(username, password) in credentials:
            connection.send(b"Welcome to socket server panel. \n")
            ...
            while True:
                ...
                if data == "server info":
                    ...
                    connection.send("Command not found: " + data + b'\n')
        else:
            connection.send(b"Access denied.")
    except socket.error:
        print("An error accured with client ip={0}, port{1}".format(*client_address))
    finally:
        connection.close()
